...
router.post(
	...
	async (req, res) => {
		...
		try {
			...
			const authToken = jwt.sign({ id: user._id, username: user.username }, key)
...
router.post(
	...
	// Validation for each value
	[body("username", "Enter username").exists(), body("password", "Passowrd cannot be blank.").exists()],
	async (req, res) => {
		const errors = validationResult(req)
...
router.put("/profile/update", getProfile, async (req, res) => {
	...
	try {
		...
		user = await Accounts.findOneAndUpdate({ _id: userID }, { $set: updatedProfile }, { new: true })
		res.send(user)
	} catch (error) {
		res.status(500).json({ message: "Some error occurred", error: error.message })
	}
...
router.put("/request/:requestee", getProfile, async (req, res) => {
	...
	try {
		...
		if (!requestee) {
			return res.status(404).json(`No user with username @${requesteeName} exists in the database.`)
		}

		if (user.friends.includes(requesteeName)) {
			return res.status(401).json(`You and @${requesteeName} are already friends.`)
		}

		if (user.requested.includes(requesteeName)) {
			return res.status(401).json(`You have already requsted.`)
		}
		if (user.blocked.includes(requesteeName)) {
		...
		user = await Accounts.findOneAndUpdate({ _id: userID }, { $set: requestsArray }, { new: true })
		...
		user = await Accounts.findOneAndUpdate({ _id: requestee._id }, { $set: RequesteeObj }, { new: true })
...
router.put("/cancelrequest/:requestee", getProfile, async (req, res) => {
	...
	try {
		...
		if (!user.requested.includes(requesteeName) || user.blocked.includes(requesteeName)) {
			return res.status(401).json(`You didnot request`)
		}
...
router.put("/declinerequest/:requestor", getProfile, async (req, res) => {
	...
	try {
		...
		if (
			!requestor.requested.includes(user.username) ||
			user.friends.includes(requestorName) ||
			user.blocked.includes(requestor)
		) {
			return res.status(401).json(`Not allowed`)
		}
		...
		requestor = await Accounts.findOneAndUpdate({ _id: requestor._id }, { $set: requestsArray }, { new: true })
		...
		user = await Accounts.findOneAndUpdate({ _id: user._id }, { $set: requestorsArray }, { new: true })
...
router.put("/handshake/:requestor", getProfile, async (req, res) => {
	...
	try {
		...
		if (!requestor) {
			return res.status(404).json(`No user with username @${requestorName} exists in the database.`)
		}

		if (!requestor.requested.includes(user.username)) {
			return res.status(401).json(`Not allowed`)
		}
		...
		if (!user.friends.includes(requestorName)) {
			user = await Accounts.findOneAndUpdate({ _id: userID }, { $set: userFriends }, { new: true })
		}

		if (!requestor.friends.includes(user.username)) {
			requestor = await Accounts.findOneAndUpdate({ _id: requestor._id }, { $set: requestorFriends }, { new: true })
		}
...
router.put("/goodbye/:friend", getProfile, async (req, res) => {
	...
	try {
		...
		if (!user.friends.includes(friend)) {
			return res.json(`You and @${friend} are not friends.`)
		}
...
router.get("/friendlist/:friend", getProfile, async (req, res) => {
	...
	try {
		...
		if (!friend) res.status(401).json("Not allowed")
		else if (friend.friends.includes(user.username)) res.send(friend.friends)
		else res.status(401).json("Not allowed")
	} catch (error) {
		res.status(500).json({ message: "Some error occurred", error: error.message })
	}
})
...
router.put("/block/:userToBeBlocked", getProfile, async (req, res) => {
	...
	try {
		...
		if (!userToBeBlocked) {
			return res.status(404).json(`No user with username @${userToBeBlockedName} exists in the database.`)
		}

		if (user.blocked.includes(userToBeBlockedName)) {
			return res.json(`You have already blocked@ ${userToBeBlockedName}.`)
		}
		...
		user = await Accounts.findOneAndUpdate({ _id: userID }, { $set: blocklist }, { new: true })
		...
		blockedUser = await Accounts.findOneAndUpdate({ _id: userToBeBlocked._id }, { $set: blockedUser }, { new: true })
...
router.put("/unblock/:userToBeUnBlocked", getProfile, async (req, res) => {
	...
	try {
		...
		if (!userToBeUnblocked) {
			return res.status(404).json(`No user with username @${userToBeUnBlockedName} exists in the database.`)
		}

		if (!user.blocked.includes(userToBeUnBlockedName) && !userToBeUnblocked.blockedBy.includes(user.username)) {
			return res.json(`You have not blocked @${userToBeUnBlockedName}.`)
		}
		...
		blockedUser = await Accounts.findOneAndUpdate({ _id: userToBeUnblocked._id }, { $set: blockedUser }, { new: true })
