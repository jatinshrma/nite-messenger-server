...
router.get("/messages/:username", getProfile, async (req, res) => {
	...
	if (!username) {
		return res
			.status(404)
			.json({ error: "This application doesn't receive messsages from ghosts, pass a valid friend's username." })
	}
...
router.post(
	...
	[body("message", "No message").exists(), body("status", "No status").exists()],
	async (req, res) => {
...
router.put("/status/:status", getProfile, async (req, res) => {
	...
	try {
		...
		if (user.username !== message.receiver) {
			...
			message = await Messages.findByIdAndUpdate(messageId, { $set: statusUpdate }, { new: true })
			res.send(message)
		}
	} catch (error) {
		res.status(500).json(error.message)
